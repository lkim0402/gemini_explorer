...
vertexai.init(project="gemini-explorer-427507", location="us-central1")

config = generative_models.GenerationConfig(
    temperature=0.4
)

# Creating the model with config
...
for index, message in enumerate(st.session_state.messages):
    content = Content(
        role=message["role"],
        parts=[Part.from_text(message["content"])]  # Creates a list with a single Part object containing the message text
    )
...
query = st.chat_input("Gemini Explorer")  # chat_input displays a chat input widget
...
#resetting button
if st.button("Reset"):
    st.session_state.messages = [] #initializing chat history
    chat = model.start_chat() #starting a new chat session
